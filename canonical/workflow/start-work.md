...
- If neither a readable plan nor the complete reviewed draft is available, report the expected path and ask for the plan or `{{ commands.plan }}`. Do not reconstruct a missing plan from a summary or invent review evidence.
...
3. Select the Developer by unresolved decisions and credible verification, not file count or position in the phase:
   ...
   - After accepting that submission, use `{{ roles.bounded }}` for follow-up tasks whose behavior, boundaries, implementation pattern, and verification are established. Extending a proven pattern across layers can qualify.
   - Send already bounded, predictable, directly verifiable work to `{{ roles.bounded }}` immediately. Do not require a preliminary complex-Developer task.
