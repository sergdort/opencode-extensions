...
- Require the resolved plan to exist. If missing, report the expected path and tell the user to run `{{ commands.plan }}`.
...
5. Validate the plan's internal references. Every phase component must exist in the architecture table, and every behavior ID in the test strategy. Correct an invalid existing plan with the user as Architect, then resume. Do not send the user to the creation-only `{{ commands.plan }}` command.
...
3. Choose `{{ roles.bounded }}` only when behavior, ownership, scope, and verification are all bounded and predictable. Use `developer` for uncertain, cross-layer, stateful, lifecycle-sensitive, debugging-heavy, broad, or weakly verified work.
...
4. Use the independent `{{ roles.review }}` agent when available. Give it the plan path, comparison range, current changes, and review focus. Keep it read-only.
