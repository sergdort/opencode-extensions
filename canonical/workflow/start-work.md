Execute the agreed outcome. This command is explicit execution; `{{ commands.plan }}` never implements anything.
...
- Delegate product code to a Developer: `developer` when design, integration, or debugging is unresolved, and `{{ roles.bounded }}` for predictable, directly verifiable work that follows an established pattern. Routing is your judgment, not choreography; reassess when evidence changes the route.
