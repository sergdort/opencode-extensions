...
- Use `plan.md` as the execution artifact. For a new plan, set `Review baseline` and `Known gate failures at baseline` to `unset`; `{{ commands.start }}` owns both fields.
- Respect native planning restrictions. If the active mode cannot write the target, keep the draft in the conversation or the harness-designated plan file. Pass the full draft text to reviewers when no readable file exists. At handoff, identify the source and intended `plan.md` path so `{{ commands.start }}` can save the reviewed plan without redesigning it.
...
Settled means no silent change, not immutable. `{{ commands.start }}` updates the plan when implementation evidence disproves a settled rule.
...
- Reference exact component names and behavior IDs. `{{ commands.start }}` rejects unresolved references.
...
Present the reviewed plan and material review changes before asking for final approval or offering `{{ commands.start }}`. Invoking `{{ commands.start }}` approves the current disclosed plan, not unseen changes or an unresolved blocking decision. Do not implement or start orchestration merely because the user approves the plan in conversation; wait for the execution command.

Use the harness's required final-plan format when one is active. Include the reviewed plan or its readable source, intended `plan.md` path, review evidence, unresolved non-blocking questions, and the next command: `{{ commands.start }}` or `{{ commands.start }} <plan-path>`. Do not claim a file was saved when planning restrictions prevented it.
