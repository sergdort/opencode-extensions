...
- Create only `plan.md`. Set `Review baseline` and `Known gate failures at baseline` to the literal value `unset`; `{{ commands.start }}` owns both fields.
...
Present material review changes before asking for final approval or offering `{{ commands.start }}`. Invoking `{{ commands.start }}` approves the current disclosed plan, not unseen changes or an unresolved blocking decision. Existing plans can be revised directly with Architect without repeating this creation command.

In the final response, reuse the plan's tables and shapes instead of writing a prose summary. Report the plan path, unresolved non-blocking seams, material review changes, and the next command: `{{ commands.start }}` or `{{ commands.start }} <plan-path>`.
