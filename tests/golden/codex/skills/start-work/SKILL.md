...
3. Select the Developer by unresolved decisions and credible verification, not file count or position in the phase:
   ...
   - After accepting that submission, use `developer_luna` for follow-up tasks whose behavior, boundaries, implementation pattern, and verification are established. Extending a proven pattern across layers can qualify.
   - Send already bounded, predictable, directly verifiable work to `developer_luna` immediately. Do not require a preliminary complex-Developer task.
